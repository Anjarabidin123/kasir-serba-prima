...
import { CheckCircle, XCircle, Ban, MessageCircle, Instagram, ArrowLeft } from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export const UserManagement = () => {
  ...
  useEffect(() => {
    if (isAdmin) {
      ...
      const profilesSubscription = supabase
        .channel('profiles_admin_channel')
        .on('postgres_changes', 
  ...
  const saveAdminContacts = async () => {
    ...
    try {
      ...
      toast.error(`Gagal menyimpan kontak admin: ${error?.message || 'Unknown error'}`);
    } finally {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      console.log('Processed users:', { pending: pending.length, approved: approved.length });

      setPendingUsers(pending);
      setApprovedUsers(approved);
    } catch (error: any) {
      console.error('Error fetching users:', error);
      toast.error(`Gagal memuat daftar user: ${error?.message || 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
